...
if __name__ == '__main__' and sys.argv[1] == 'train':
    ...
    #加载数据集
    input_images = np.load('preprocess_data/digits/digits_input_images.npy')
    ...
    with tf.Session() as sess:
        ...
        for it in range(iterations):
            ...
            for n in range(batches_count):
                _, c = sess.run([train_step,cross_entropy], feed_dict={x: input_images[n * batch_size:(n + 1) * batch_size],
                                          y_: input_labels[n * batch_size:(n + 1) * batch_size], keep_prob: 0.5})
                avg_cost += c / batches_count

            if remainder > 0:
                start_index = batches_count * batch_size
                _, c = sess.run([train_step,cross_entropy], feed_dict={x: input_images[start_index:input_count - 1],
            ...
            if it % 5 == 0:
                ...
                if iterate_accuracy >= 0.95 and it >= iterations:
                    break
            ax1.plot(it + 1, iterate_accuracy , 'mo')
            ax2.plot(it + 1, avg_cost, 'co')
            print(avg_cost)
        print('完成训练!')
        time_elapsed = time.time() - time_begin
        ...
        plt.suptitle(
            "learning rate=%f, training epochs=%i " % (learning_rate, iterations),
            size=15)
